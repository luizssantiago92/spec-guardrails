...
export async function runDoctorChecks(cwd) {
  ...
  if (companions.length > 0) {
    for (const companion of companions) {
      const gateOk = await pathExists(
        cwd,
        path.join(companion.scriptsDir, companion.gates[0] ?? ""),
      );
      checks.push({
        id: `atlas-${companion.id}`,
        label: `${companion.displayName} (${companion.npm}@${companion.version})`,
        weight: 3,
        pass: gateOk,
        optional: true,
        suggest: gateOk
          ? undefined
          : `npx ${companion.npm} install`,
      });
    }
  }
