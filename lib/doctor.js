...
/**
 * True when `command` resolves on PATH (binary exists). Non-zero exit from
 * `--version` still counts as available — only ENOENT means missing.
 *
 ...
 * @returns {string}
 */
function rtkInitSuggest(adapterIds) {
  const init =
    adapterIds.includes("cursor")
      ? "rtk init -g --agent cursor"
      : "rtk init -g";
  return `Optional: install RTK (https://github.com/rtk-ai/rtk) then \`${init}\` — compresses bash output for agents; does not affect Guardrails score`;
}
...
export async function runDoctorChecks(cwd) {
  ...
  const rtkOk = await commandOnPath("rtk");
  checks.push({
    id: "rtk-available",
    label: "RTK CLI (optional bash-output compression)",
    weight: 1,
    pass: rtkOk,
    optional: true,
    suggest: rtkOk ? undefined : rtkInitSuggest(adapterIds),
  });

  const graphifyCli = await commandOnPath("graphify");
  const graphifyOut = await pathExists(cwd, "graphify-out");
  const graphifyOk = graphifyCli || graphifyOut;
  checks.push({
    id: "graphify-available",
    label: "Graphify (optional brownfield graph/wiki)",
    weight: 1,
    pass: graphifyOk,
    optional: true,
    suggest: graphifyOk
      ? undefined
      : "Optional: install Graphify (https://github.com/Graphify-Labs/graphify) for large-repo wiki/graph navigation — keep code-index for shallow lookup; does not affect Guardrails score",
  });
